!function () {
  ...
  Arboreal.parse = function (object, childrenAttr, getId) {
    ...
    (function walkDown(node, parent) {
      ...
      // Use supplied function to generate node id
      id = getId ? getId(node, parent.id) : null;

      if (!parent) {
        newNode = root = new this.constructor(null, getNodeData(node), id);
      } else {
        newNode = new this.constructor(parent, getNodeData(node), id);
  ...
  Arboreal.prototype.appendChild = function (data, id) {
    var child = new this.constructor(this, data, id);
